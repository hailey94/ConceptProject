...
class AwADataset(Dataset):
    def __init__(self, args, train=True, transform=None):
        ...
        self.root = os.path.join(args.dataset_dir, "AwA2")
    ...
    def _get_path_label(self):
        for line in open(self.images_file):
            ...
            if label not in self._cls_name_dict.keys():
                cls_name = image_name.split('/')[-2]
                self._cls_name_dict[int(label)-1] = cls_name
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.root, 'JPEGImages', image_name)
    ...
    def __len__(self):
        if self.train:
            return len(self._train_path_label)
        else:
            return len(self._test_path_label)
