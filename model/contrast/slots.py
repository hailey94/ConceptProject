...
class ScouterAttention(nn.Module):
    def __init__(self, args, dim, num_concept, iters=3, eps=1e-8, vis=False, power=1, to_k_layer=3, index=None):
        super().__init__()
        self.args = args
    ...
    def forward(self, inputs_pe, inputs, weight=None, things=None):
        ...
        if self.vis:
            ...
            vis(slots_vis_raw, "vis", self.args.feature_size, weight, things, self.index)
        return updates, attn


def vis(slots_vis_raw, loc, size, weight=None, things=None, index=None):
    ...
    for i in range(b):
        ...
        if weight is not None:
            ...
            overall.save(f'{loc}/{index}/overall_{Nos}.png')
            ...
            for id, image in enumerate(slots_vis):
                ...
                if things is not None:
                    ...
                    loc2 = f"vis_pp/{index}/cpt{cpt_num}/"
                    if id == cpt_num:
                        image.save(loc2 + f'mask_{order}_{category}.png')
                        break
                    else:
                        continue
                image.save(f'{loc}/{index}/{i}_slot_{id:d}.png')
